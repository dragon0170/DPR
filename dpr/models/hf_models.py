...
if transformers.__version__.startswith("4"):
    from transformers import BertConfig, BertModel, RobertaModel, RobertaConfig, XLMRobertaModel, XLMRobertaConfig
...
def get_xlm_roberta_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    dropout = cfg.encoder.dropout if hasattr(cfg.encoder, "dropout") else 0.0
    question_encoder = HFXlmRobertaEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    ctx_encoder = HFXlmRobertaEncoder.init_encoder(
        cfg.encoder.pretrained_model_cfg,
        projection_dim=cfg.encoder.projection_dim,
        dropout=dropout,
        pretrained=cfg.encoder.pretrained,
    ...
    tensorizer = get_xlm_roberta_tensorizer(cfg)
    return tensorizer, biencoder, optimizer
...
def get_xlm_roberta_tensorizer(cfg):
    sequence_length = cfg.encoder.sequence_length
    pretrained_model_cfg = cfg.encoder.pretrained_model_cfg
    tokenizer = get_xlm_roberta_tokenizer(pretrained_model_cfg, do_lower_case=cfg.do_lower_case)
    if cfg.special_tokens:
        _add_special_tokens(tokenizer, cfg.special_tokens)
...
def get_xlm_roberta_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True):
    # still uses HF code for tokenizer since they are the same
...
class HFXlmRobertaEncoder(XLMRobertaModel):
    ...
    def init_encoder(
        ...
        if pretrained:
            ...
            return HFXlmRobertaEncoder(cfg, project_dim=projection_dim)
